...
locals {
  ...
  region            = var.aws_region
  s3_backend_bucket = var.s3_bucket_name
...
resource "aws_db_instance" "rds" {
  ...
  tags = merge(local.common_tags, { Name = "RDS-Instance" })
}

resource "aws_db_subnet_group" "rds_subnet_group" {
  name       = "rds-subnet-group"
  subnet_ids = [aws_subnet.subnets["private_1"].id, aws_subnet.subnets["private_2"].id]
